...
def generate_daily_stats_plot(daily_summary_melted):
    ...
    title_font_size=24)
    fig.write_html("app/static/daily_stats.html",include_plotlyjs='cdn')
def generate_commute_delay_plot(commute_delay_counts):
    ...
    margin=dict(l=20, r=20, t=50, b=20),
    title_font_size=24)
    fig_commute_delay.write_html("app/static/commute_delay.html",include_plotlyjs='cdn')
def generate_delay_minutes_plot(unique_trips):
    ...
    margin=dict(l=20, r=20, t=50, b=20),
    title_font_size=24)
    fig_delay_minutes.write_html("app/static/delay_minutes.html",include_plotlyjs='cdn')
def save_graphs(df):
...
def main():
    ...
    with open("app/static/on_time_percentage.txt", "w") as f:
        f.write(f"{on_time_percentage:.2f}%")
    with open("app/static/stop_date.txt", "w") as f:
        f.write(f"{stop_date}")
    with open("app/static/start_date.txt", "w") as f:
        f.write(f"{start_date}")
    with open("app/static/n_datapoints.txt", "w") as f:
